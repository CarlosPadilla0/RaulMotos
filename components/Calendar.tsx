...
export const Calendar: React.FC<CalendarProps> = ({ selectedDate, onDateSelect }) => {
  ...
  const handleDateClick = (day: number) => {
    ...
    date.setHours(0,0,0,0);
    if (date >= today && date <= lastAvailableDate && date.getDay() !== 0) { // Sunday check
  ...
  const renderDays = () => {
    ...
    for (let day = 1; day <= daysInMonth; day++) {
        ...
        const isSuggested = loopDate.getTime() === suggestedDate.getTime() && !selectedDate;
  ...
  return (
    <div className="bg-white p-4 rounded-lg border">
      <div className="flex justify-between items-center mb-4">
        <button onClick={() => changeMonth(-1)} className="p-1 rounded-full hover:bg-gray-100 disabled:opacity-50" disabled={currentDate.getMonth() === today.getMonth() && currentDate.getFullYear() === today.getFullYear()}>
